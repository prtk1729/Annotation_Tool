...
args = Namespace(is_os_win=0, initials='hk')
...
# When not hosting comment the line below
server = app.server
...
def gen_cards(current_18,  no_func=False):
    ...
    return [
        dbc.Row([
            dbc.Col([card(i)], width=2) for i in gl_current_18[:6]]),
        dbc.Row([
            dbc.Col([card(i)], width=2) for i in gl_current_18[6:12]]),
        dbc.Row([
            dbc.Col([card(i)], width=2) for i in gl_current_18[12:18]]),
    ]
...
def save_labels_uptil_now(n_images, ordered_file_name_list, div_by_6=False):
    ...
    day, month, year = today.day, today.month, today.year
    if div_by_6 == False:
        print('\nFrom div_by_6 of save_labels_uptil_now if-part\n')
        ...
        print('\nFrom else of save_labels_uptil_now ')
        # current_18 = []
        # unseen_idx_set = unseen_idx_set.difference(set(current_18))
        # ssil = list(unseen_idx_set)
        temp_current_18 = list(unseen_idx_set)[:18]
        unseen_idx_set = unseen_idx_set.difference(set(temp_current_18))
        ssil = list(unseen_idx_set)
        print("ssil-Line_609: ", ssil)
        ...
        print('DICT:', images_annotated_today_dict)
...
def save_functionality2MM(ordered_file_name_list):
    ...
    #   modify class_of_all_images before writing
    for i in range(len(gl_current_18)):
        print("save_functionality gl_current_18==> modifies class_of_all_images: ",gl_current_18)
        class_of_all_images[gl_current_18[i]] = gl_state_18[i]
...
def next(n_clicks):
    ...
    if( (n_clicks % 6 == 0) ):
        print('Inside if of next')
        print(f"\nNext is clicked: {n_clicks}th time; Session_num: {session_num}\n")

        # Here I I have all the image_names and their labels
        # All I need to do is NOT do the same opns as done in "else" i.e
        # no set difference and save the dict to df and json
        # Focus on what makes the net set of images show up
        # save_labels_uptil_now(len(paths_of_images), ordered_file_name_list, div_by_6=True)
        ...
        print('Inside else of next')
        print(f"\nNext is clicked: {n_clicks}th time; Session_num: {session_num}\n")
        initial_state_18 = [0 for i in range(18)]
        plot_grid_session_iter_num += 1

        if plot_grid_session_iter_num >= 2:
            save_functionality2MM(ordered_file_name_list)
...
def stop_session(n_clicks):
    ...
    if n_clicks == 1:
        ...
        # save_everything uptil now and then modify_session_file_fot_the_day
        save_labels_uptil_now(len(paths_of_images), ordered_file_name_list, div_by_6=True)
